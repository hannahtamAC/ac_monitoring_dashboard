...
export const CompactFlightCard = ({
  ...
  const getBoardingProgress = () => {
    return Math.round(flight.boardedPassengers / flight.totalPassengers * 100) || 0;
  };
  const getBaggageProgress = () => {
    return Math.round(flight.loadedBags / flight.totalBags * 100) || 0;
  };
  return <div className={`bg-white border rounded-lg p-3 ${getCardClassName()}`}>
      <div className="grid grid-cols-12 gap-2 items-center">
        ...
        <div className="col-span-3 space-y-2">
          <div>
            ...
            <ProgressBar value={getBoardingProgress()} status={flight.status} startTime={flight.boardingStartTime} completeTime={flight.boardingCompleteTime} type="boarding" />
          </div>
          <div>
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center text-xs text-gray-500">
