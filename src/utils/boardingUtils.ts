export const getBoardingStartTime = flight => {
  ...
  if (flight.aircraftType.includes('777')) {
    ...
    boardingMinutes = 45;
  } else if (flight.aircraftType.includes('737') || flight.aircraftType.includes('A320') || flight.aircraftType.includes('A321')) {
    boardingMinutes = 35;
  }
...
export const isBoardingLate = flight => {
  if (flight.status === 'departed' || flight.status === 'scheduled') return false;
  const now = new Date();
